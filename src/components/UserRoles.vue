<template>
  ...
  <div class="col-12">
    ...
    <div v-if="!loadTable" class="table-responsive">
      <table  class="table table-sm caption-top" id="user_roles_table">
        <caption>
          <div class="row">
            <div class="col">
        ...
        <thead class="table-dark">
          <tr>
            <th class="border-end border-light">IdUser</th>
            <th class="border-end border-light">IdRole</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(user,index) in jsonData" :key="index" :class="`${index}ROW`">
            <td class="border-start border-end border-secondary">{{ user["IdUser"] }}</td>
            <td class="border-end border-secondary">{{ user["IdRole"] }}</td>
          </tr>
        </tbody>
      </table>  
    </div>        
...
export default {
      ...
      methods: {
          ...
          async clickUpload() {
              try {
                  ...
                  await this.jsonData.forEach(async (item, index) => {
                      let inputData = {
                          "IdUser": parseFloat(item["IdUser"] ? item["IdUser"] : 0),
                          "IdRole": parseFloat(item["IdRole"] ? item["IdRole"] : 0),
                          
                      }
                      let result = await RequestOnlyInfo("UserRoles", "AddUserRoles", inputData,this.$root.orgByPlant,this.$root.email)    
                      count += 1 
                      if (result) {
          ...
          exportExcelOnlyHTML() {
                const data = document.getElementById(`user_roles_table`).cloneNode(true)                
                const wb = XLSX.utils.table_to_book(data, { sheet: `template` })
                XLSX.writeFile(wb, ((`user_roles_table.xlsx`)))
          },
      }
}
</script>
