<template>
  ...
  <div class="col-12">
    ...
    <div v-if="!loadTable" class="table-responsive">
      <table  class="table table-sm caption-top" id="wip_table">
        <caption>
          <div class="row">
            <div class="col">
        ...
        <thead class="table-dark">
          <tr>
            <th class="border-end border-light">JobName</th>
            <th class="border-end border-light">Quantity</th>
            <th class="border-end border-light">ItemName</th>
            <th class="border-end border-light">RouteChainName</th>
            <th class="border-end border-light">RouteSeq</th>
            <th class="border-end border-light">RouteName</th>
            <th class="border-end border-light">StepName</th>
            <th class="border-end border-light">JobStatus</th>
            <th class="border-end border-light">Priority</th>
            <th class="border-end border-light">UserName</th>
            
          </tr>
        </thead>
        <tbody>
          <tr v-for="(user,index) in jsonData" :key="index" :class="`${index}ROW`">
            <td class="border-start border-end border-secondary">{{ user["JobName"] }}</td>
            <td class="border-end border-secondary">{{ user["Quantity"] }}</td>
            <td class="border-end border-secondary">{{ user["ItemName"] }}</td>
            <td class="border-end border-secondary">{{ user["RouteChainName"] }}</td>
            <td class="border-end border-secondary">{{ user["RouteSeq"] }}</td>
            <td class="border-end border-secondary">{{ user["RouteName"] }}</td>
            <td class="border-end border-secondary">{{ user["StepName"] }}</td>
            <td class="border-end border-secondary">{{ user["JobStatus"] }}</td>
            <td class="border-end border-secondary">{{ user["Priority"] }}</td>
            <td class="border-end border-secondary">{{ user["UserName"] }}</td>
...
export default {
  ...
  methods: {
          ...
          try {
              ...
              await this.jsonData.forEach(async (item, index) => {
                  ...
                  let inputData = {
                      ...
                      "RouteChainName": item["RouteChainName"]?.trim() || '',
                      "RouteSeq": parseFloat(item["RouteSeq"] ? item["RouteSeq"] : 0),
                      "RouteName": item["RouteName"]?.trim() || '',
                      "StepName": parseFloat(item["StepName"] ? item["StepName"] : 0),
                      "JobStatus": item["JobStatus"]?.trim() || '',
                      "Priority": parseFloat(item["Priority"] ? item["Priority"] : 0),
                  }
                  let result = await RequestOnlyInfo("MassiveUpload", "SimpleCreateJobSimple", inputData,this.$root.orgByPlant,this.$root.email)   
                  if (result) {
      ...
      exportExcelOnlyHTML() {
            const data = document.getElementById(`wip_table`).cloneNode(true)                
            const wb = XLSX.utils.table_to_book(data, { sheet: `template` })
            XLSX.writeFile(wb, ((`wip_table.xlsx`)))
      },
  }
}
</script>
