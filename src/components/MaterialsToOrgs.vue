<template>
  <h2 class="mb-4">MATERIALS TO ORGS</h2>
  <div class="col-4">
    <div class="mb-3">
  ...
  <div class="col-12">
    ...
    <div v-if="!loadTable" class="table-responsive">
      <table  class="table table-sm caption-top" id="materialsToOrgs_table">
        <caption>
          <div class="row">
            <div class="col">
        ...
        <thead class="table-dark">
          <tr>
            ...
            <th class="border-end border-light">ChangedBy</th>
        ...
        <tbody>
          <tr v-for="(user,index) in jsonData" :key="index" :class="`${index}ROW`">
            ...
            <td class="border-end border-secondary">{{ user["ChangedBy"] }}</td>
...
export default {
  ...
  methods: {
          ...
          try {
              ...
              await this.jsonData.forEach(async (item, index) => {
                  ...
                  let inputData = {
                      ...
                      "ChangedBy": item["ChangedBy"]?.trim() || '',
                      "MaterialId": parseFloat(item["MaterialId"] ? item["MaterialId"] : 0),
                  }
                  let result = await RequestOnlyInfo("MaterialbyOrganization", "AddMaterialbyOrganizations", inputData,this.$root.orgByPlant,this.$root.email)   
                  if (result) {
      ...
      exportExcelOnlyHTML() {
            const data = document.getElementById(`materialsToOrgs_table`).cloneNode(true)                
            const wb = XLSX.utils.table_to_book(data, { sheet: `template` })
            XLSX.writeFile(wb, ((`materialsToOrgs_table.xlsx`)))
      },
  }
}
</script>
